...
export const validateFilaDescargaData = (response: any): any => {
  ...
  if (!isArray(dados.listaFilaDescarga.filaDescargaVeiculos)) {
    console.warn(
      "[ApiValidator] filaDescargaVeiculos não é array, normalizando"
    );
...
export const validatePatioData = (
  response: any,
  tipo: "descarga" | "carga" = "descarga"
): any => {
  validateVehicleData(response, `Patio${tipo}Data`);

  const { dados } = response;
  const listKey =
    tipo === "descarga" ? "listaPatioDescarga" : "listaPatioCarga";
  const veiculosKey =
    tipo === "descarga" ? "patioDescargaVeiculos" : "patioCargaVeiculos";
...
export const validateOperacoesData = (
  response: any,
  tipo: "descarga" | "carga" = "descarga"
): any => {
  ...
  const veiculosKey =
    tipo === "descarga" ? "DescargaVeiculos" : "CargaVeiculos";
...
export const validateTransportData = (data: any): ValidationResult => {
  ...
  try {
    if (data.patioDescargaData)
      validatePatioData(data.patioDescargaData, "descarga");
  } catch (error) {
  ...
  if (errors.length > 0) {
    console.warn(
      "[ApiValidator] Erros na validação de dados de transporte:",
      errors
    );
  }
