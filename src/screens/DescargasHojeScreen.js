...
const DescargaScreen = ({ navigation, route }) => {
  ...
  const [descargaData, setDescargaData] = useState([]);
  ...
  const fetchDescargaData = async () => {
    try {
      ...
      } else if (data.dados?.listaDescarga?.DescargaVeiculos) {
        const descargaData = data.dados.listaDescarga.DescargaVeiculos;
        setDescargaData(descargaData);

        // Calcular totais
        const totalV = descargaData.reduce(
          (sum, item) => sum + (item.d_veiculos || 0),
          0
        );
        const totalP = descargaData.reduce(
          (sum, item) => sum + (item.d_peso || 0),
          0
        );
      ...
      console.error("Erro ao buscar dados de descarga:", error);
  ...
  const renderDescargaItem = ({ item }) => (
    <View style={styles.descargaCard}>
      <View style={styles.cardHeader}>
        <Text style={styles.filaText}> {item.d_grupo || "Não informado"}</Text>
        <View style={styles.veiculosBadge}>
          <Text style={styles.veiculosText}>{item.d_veiculos} veíc.</Text>
        </View>
      </View>
      ...
      <View style={styles.cardContent}>
        ...
        <View style={styles.infoRow}>
          ...
          <Text style={styles.infoValue} numberOfLines={1} ellipsizeMode="tail">
            {item.d_produto || "Não informado"}
          </Text>
        </View>

        <View style={styles.infoRow}>
          <Text style={styles.infoLabel}>Peso:</Text>
          <Text style={styles.infoValue}>{formatPeso(item.d_peso || 0)}</Text>
        </View>
      </View>
    </View>
  );
  ...
  const renderHeader = () => (
    <View>
      ...
      <View style={styles.summaryContainer}>
        ...
        <View style={styles.summaryDivider} />
        <View style={styles.summaryItem}>
          <Text style={styles.summaryValue}>{descargaData.length}</Text>
    ...
    <SafeAreaView style={styles.container}>
      ...
      <View style={styles.header}>
        ...
        <View style={styles.headerCenter}>
          <Text style={styles.headerTitle}>Descargas do Dia</Text>
      ...
      {/* Lista de Veículos em Descarga com resumo no header */}
      {loading && descargaData.length === 0 ? (
          ...
          ListEmptyComponent={
            <View style={styles.emptyContainer}>
              ...
              <Text style={styles.emptyText}>
                {errorMessage || "Nenhuma descarga encontrada"}
              </Text>
