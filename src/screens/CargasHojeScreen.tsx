...
type Props = StackScreenProps<RootStackParamList, "CargasHoje">;

const CargasHojeScreen: React.FC<Props> = ({ navigation }) => {
  const { state } = useApp();
  ...
  const keyExtractor = useCallback((item: any, index: number) => {
    ...
    if (grupo && produto) {
      return `${grupo}-${produto}-${peso}-${veiculos}-${data}-${hora}`;
    }
    if (fila && produto) {
      return `${fila}-${produto}-${peso}-${veiculos}-${data}-${hora}`;
    }
    return `item-${grupo || fila}-${produto}-${data}-${hora}-${index}`;
  }, []);
  ...
  const renderEmptyComponent = useCallback(() => {
    ...
    return (
      <EmptyView
        icon="cube-outline"
        message={error || "Nenhuma carga realizada hoje"}
  ...
  if (loading && (!data || data.length === 0)) {
    return (
      <SafeAreaView style={styles.container}>
        ...
        />
        <LoadingSpinner text="Carregando cargas de hoje..." />
      </SafeAreaView>
    );
  }
  ...
  return (
    <SafeAreaView style={styles.container}>
      ...
      <BackgroundLoadingIndicator
        ...
        text="Atualizando cargas de hoje..."
        position="bottom"
      />
    </SafeAreaView>
  );
