...
type Props = StackScreenProps<RootStackParamList, "DescargasHoje">;

const DescargasHojeScreen: React.FC<Props> = ({ navigation }) => {
  const { state } = useApp();
  ...
  const { data, loading, refreshing, totals, error, refresh, lastUpdate } =
    useMonitorData("monitor_descarga", state.selectedFilial, apiFilters);
