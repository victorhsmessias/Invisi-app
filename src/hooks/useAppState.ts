...
interface UseAppStateReturn {
  ...
  updateActivity: () => void;
  onAppStateChange: (callback: (context: AppStateChangeContext) => void) => () => void;
}
...
export const useAppState = (): UseAppStateReturn => {
  ...
  const isUserIdle = useCallback((thresholdMs: number = USER_IDLE_THRESHOLD): boolean => {
  ...
  const onAppStateChange = useCallback((callback: (context: AppStateChangeContext) => void) => {
